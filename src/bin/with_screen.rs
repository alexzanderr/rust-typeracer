...
fn main() {
    ...
    screen.print(&text, 2, 1);
    screen.print("^", 3, 1);
    screen.print("│", 4, 1);
    screen.print(format!("Index: {index}").as_str(), 6, 1);
    screen.print(format!("Wrong: {wrong_index}").as_str(), 7, 1);
    screen.print(
        format!("Index + Wrong: {}", index + wrong_index).as_str(),
        8,
        1
    );
    screen.print(&get_text_colored(&text, index, wrong_index), 2, 1);

    if !skip {
        screen.print("^", 3, index + wrong_index + 1);
        screen.print("│", 4, index + wrong_index + 1);
    }
    screen.print(format!("text len: {}", text.len()).as_str(), 9, 1);
    screen.print(
    ...
    for event in stdin.events() {
        ...
        match event {
            ...
            Event::Key(Key::Char(character)) => {
                skip = false;
                screen.print(character.to_string().as_str(), 10, 1);
                    ...
                    && wrong_index == 0
                {
                    ...
                    if index + wrong_index < text.len() {
                        ...
                        screen
                            .print("you cant go further, sorry", y, x)
                            .refresh();
                        skip = true;
                    }
                }
                screen.print(character.to_string().as_str(), 5, 1);
            },
            _ => {}
        }
        screen.print(format!("Index: {index}").as_str(), 6, 1);
    ...
    screen
        .print(&get_text_colored(&text, index, wrong_index), 2, 1)
        .refresh();
